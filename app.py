...
with open("style.css") as f1:
    design=f1.read()

with open("main.min.js") as f2:
    script1=f2.read()

with open("anime.min.js") as f3:
    script2=f3.read()

with open("scrollreveal.min.js") as f4:
    script3=f4.read()
            ...
            <section class="hero">
                <div class="container">
                    <div class="hero-inner">
						<div class="hero-copy">
	                        <h1 class="hero-title mt-0">Bigmart Sales Prediction</h1>
	                        <p>Welcome to Big Mart Sales Prediction!<br>
                            Our sales prediction services are designed to help our customers make informed decisions about their inventory, pricing, and marketing strategies. We provide customized reports that include detailed sales predictions and recommendations on how to optimize sales performance.
                            </p>
						</div>
...
pickle_in = open("C:\\Users\\INFTLAPTOP-DL9\\Desktop\\Mini Project Final1--sem6\\Mini Project Final\\RF.pkl","rb")
...
def predict_sale_authentication(Item_Weight,Item_Type,Item_MRP,Fat_Content,Outlet_Type):
    
   
    prediction=regressor.predict([[Item_Weight,Item_Type,Item_MRP,Fat_Content,Outlet_Type]])
    print(prediction)
    return prediction
...
def main():
    st.title("Start Your Prediction By Answering a Few Questions.....")
...
#Fat_content=st.radio(" Select Fat Content",["Low Fat","Regular Fat","High Fat"])
    Item_type=st.selectbox(" Select Item Type ",["1 Baking Goods","2 Breads","3 Breakfast","4 Canned","5 Dairy","6 Fruits and Vegetables","7 Hard Drinks","8 Health and Hygiene","9 Household","10 Meat","11 Others","12 Seafood","13 Snack Foods","14 Soft Drinks","15 Starchy Foods"])
    IT = Item_type.split(" ")
    model_pass = IT[0]
    Item_mrp=st.number_input(" Enter Item MRP")
    Fat_Content=st.selectbox("Fat Content",["1 low","2 High","3 Regular"])
    OS = Fat_Content.split(" ")
    model_pass2 = OS[0]
    Outlet_Type=st.radio(" Select Outlet Type",["1 Grocery Store","2 Supermarket Type 1","3 Supermarket Type 2"])
    OT = Outlet_Type.split(" ")
    model_pass3= OT[0]
   
#Outlet_Location=st.selectbox("Outlet Location Type",["Tier 1","Tier 2","Tier 3"])
    result=""
    if st.button("Predict"):
        result=predict_sale_authentication(Item_weight,model_pass,Item_mrp,model_pass2,model_pass3)
    #st.success(result)
...
with col5:
    pass
with col3 :
	b=st.button("Thank you for Choosing Big Mart Prediction")